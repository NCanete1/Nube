...
class ValidadorRutTest {
    ...
    public void NormalizarTest(){
        ...
        for(int i= 0;i<rut.length;i++){
            String normalizado = ValidadorRut.Normalizar(rut[i]);
    ...
    public void DigitoTest(){                                          //Que de el D.Verificador (Numero) 10=k, 11=0
        ...
        boolean condicion = true;
        for(int i= 0;i<rut.length;i++){
            int digitoTest = ValidadorRut.Digito(rut[i]);
    ...
    public void DigitoK0(){                     //Digito verificador K y 0
        ...
        boolean condicion = true;
        for(int i= 0;i<rut.length;i++){
            String digitoTest = ValidadorRut.DigitoVerificador(rut[i]);
    ...
    public void InversoTest(){
        ...
        boolean condicion = true;
        for(int i= 0;i<rut.length;i++){
            String digitoTest = ValidadorRut.Invertir(rut[i]);
            if(!digitoTest.equals(digitos[i])){
                condicion= false;
            }
        }
    ...
    public void RutTest(){
        ...
        boolean condicion = true;
        for(int i= 0;i<rut.length;i++){
            String digitoTest = ValidadorRut.Rut(rut[i]);
            if(!digitoTest.equals(digitos[i])){
                condicion= false;
            }
        }
